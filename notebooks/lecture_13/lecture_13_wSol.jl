...
begin
	...
	with_terminal() do
		printred("autocorrelations and their t-stats:\n")
...
let
	df = DataFrame()
	for lag=1:5	
		df_l = combine(FF, names(FF,Between("SMALL LoBM","BIG HiBM")) .=> (x->(autocor(x,[lag]), sqrt(T)*autocor(x,[lag]))), renamecols = false)
		df = vcat(df,df_l)
	end
...
let
	x  = [ones(T-1) R[1:end-1]] #returns of large value firms
...
let
	df = DataFrame()
	for col in names(FF,Between("SMALL LoBM","BIG HiBM"))
		Q = Array(FF[1:end-1,col]) .< 0        #dummy: 1 if R(t-1) < 0
...
begin
	...
	for t = 2:T         #loop over periods, save portfolio returns
		...
		s                = sortperm(Rx[t-1,:])  #s[1] is the index of the worst asset
		w                = zeros(n) #recall that n is the number of porfolios
		w[s[1:5]]       .= -1/5
		w[s[end-4:end]] .= 1/5
		Rp[t]            = w'Rx[t,:]
